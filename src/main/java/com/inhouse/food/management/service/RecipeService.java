...
/**
 * The RecipeService class provides methods to manage and interact with recipes,
 * including adding recipes, retrieving all recipes, and filtering recipes based
 * on available grocery items.
 */
@NoArgsConstructor
public class RecipeService {
     ...
     * This method filters the recipes based on whether expired groceries should be included.
     *
    ...
    public List<Recipe> getPossibleRecipes(List<Grocery> fridgeItems, String includeExpiredGrocery) {
        ...
        Map<String, List<Grocery>> expiredItemsIncludedBasedOnFlag = fridgeItems.stream().filter(grocery -> {
        ...
        return cookbookForRecipes.getRecipes().stream()
                .filter(recipe -> recipe.getIngredients().entrySet().stream()
                        .allMatch(entry -> {
                            List<Grocery> grocery = expiredItemsIncludedBasedOnFlag.get(entry.getKey());
                            double availableQuantity = Optional.ofNullable(grocery).orElse(new ArrayList<>()).stream().mapToDouble(Grocery::getQuantity).sum();
